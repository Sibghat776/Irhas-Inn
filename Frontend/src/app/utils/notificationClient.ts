...
export async function subscribe(): Promise<boolean> {
  ...
  if (Notification.permission === "denied") {
    throw new Error(
      "Notification permission previously denied. Please enable it in your browser settings."
    );
  }
...
export async function unsubscribe(): Promise<boolean> {
  ...
  try {
    ...
    console.error("❌ Backend unsubscribe error", errMsg);

    throw new Error(`Failed to unsubscribe: ${errMsg}`);
  }
}