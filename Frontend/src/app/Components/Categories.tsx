...
const ProductCards: React.FC = () => {
  ...
  return (
    <section
      id="collection"
      className="py-20 bg-[#F7F7FA] transition-colors duration-500"
    >
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
          ...
          <h2 className="text-3xl md:text-5xl font-black text-[#041241] tracking-tight">
            Explore Our <span className="bg-gradient-to-r from-[#061F95] to-[#0856DF] bg-clip-text text-transparent">Collections</span>
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {categories.length === 0 ? (
            <p className="text-gray-500 text-sm col-span-full text-center">Loading categories...</p>
          ) : (
            categories.map((cat: any) => (
              <Link
                href={`/productsPage?category=${cat._id}`}
                key={cat._id}
                className="relative group rounded-2xl overflow-hidden bg-white border border-gray-100 shadow-md hover:shadow-2xl transition-all duration-500 ease-in-out flex flex-col h-[380px]"
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 z-20 flex flex-col justify-end transform transition-transform duration-500">
                  <span className="text-[#EDAE17] text-[10px] font-bold tracking-widest uppercase mb-1 opacity-90">
                  ...
                  <div className="overflow-hidden max-h-0 opacity-0 group-hover:max-h-12 group-hover:opacity-100 transition-all duration-500 ease-in-out">
                    <div className="flex items-center gap-2 text-xs font-bold text-white/90 border-t border-white/20 pt-3">
                      ...
                      <ArrowUpRight size={14} className="transform transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 text-[#EDAE17]" />
                    </div>
                  </div>
                </div>

                <div className="absolute inset-0 border-2 border-transparent group-hover:border-[#0856DF]/40 rounded-2xl transition-all duration-500 pointer-events-none z-30" />
              </Link>
            ))
          )}
        </div>
