...
function AISearchResultsInner() {
  ...
  if (!query) {
    return (
      <div className="p-4 text-center">
        <p className="text-[#222831] text-sm sm:text-base">No search query provided.</p>
      </div>
    );
  }
  ...
  if (error || !data || data.products.length === 0) {
    return (
      <div className="p-4 text-center">
        <p className="text-[#222831] text-sm sm:text-base">
          ❌ No products found for "{query}".
        </p>
        <button
          onClick={() => fetchSearchResults(query)}
          className="mt-3 px-4 py-2 bg-[#FFFFFF] text-white rounded hover:bg-[#EEEEEE] text-sm sm:text-base"
        >
          🔄 Retry Search
        </button>
      </div>
  ...
  return (
    <div className="px-3 py-4 sm:p-4 md:p-6 overflow-x-hidden">
      <div className="mb-4 sm:mb-6 mt-16 sm:mt-16 md:mt-14">
        ...
        <p className="text-[#222831] text-sm sm:text-base">
          Found <span className="font-semibold text-[#222831]">{data.count}</span> product(s) matching your query
        </p>
        {data.intent && (
          <p className="text-xs sm:text-sm text-[#222831] mt-2 break-words">
      ...
      <div className="mt-6 sm:mt-8 text-center">
        <button
          onClick={() => fetchSearchResults(query)}
          className="px-5 py-2.5 sm:px-6 sm:py-2 bg-[#FFFFFF] text-[#222831] rounded hover:bg-[#EEEEEE] transition text-sm sm:text-base min-h-[44px]"
        >
          🔄 Refresh Results
        </button>
      </div>
...
export default function AISearchResults() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FFFFFF] via-white to-white">
      <Suspense fallback={<div className="p-4 text-center text-[#222831] text-sm sm:text-base">Loading…</div>}>
