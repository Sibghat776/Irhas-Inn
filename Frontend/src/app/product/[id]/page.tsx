...
const ProductDetailPage: React.FC = () => {
  ...
  const handleAddToCart = async () => {
    ...
    const currentCart = getLocalCart() as Array<ProductDetail & { quantity: number }>;
    ...
    <main className="min-h-screen bg-gradient-to-b from-[#FFFFFF] pt-12 via-white to-[#FFFFFF] text-[#222831]">
      <div className="mx-auto max-w-7xl px-3 py-8 sm:px-6 sm:py-20">
        <button
          type="button"
          onClick={() => router.back()}
          className="mb-4 inline-flex items-center gap-1.5 rounded-xl border border-[#EEEEEE] bg-white px-3 py-2 text-xs font-semibold text-[#222831] shadow-sm transition hover:bg-[#222831] hover:text-white sm:mb-8 sm:gap-2 sm:rounded-2xl sm:px-4 sm:py-3 sm:text-sm"
        >
        ...
        {loading ? (
          <div className="rounded-2xl border border-[#EEEEEE] bg-white p-6 text-sm shadow-lg sm:rounded-[32px] sm:p-12 sm:text-base">
            Loading product details...
          </div>
        ) : error || !product ? (
          <div className="rounded-2xl border border-[#EEEEEE] bg-[#FFFFFF] p-6 text-sm text-[#222831] shadow-lg sm:rounded-[32px] sm:p-12 sm:text-base">
          ...
          <>
            <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr] lg:gap-10">
              <div className="space-y-4 rounded-2xl bg-white p-3 shadow-[0_10px_40px_rgba(0,0,0,0.06)] sm:space-y-6 sm:rounded-[32px] sm:p-6 sm:shadow-[0_20px_70px_rgba(0,0,0,0.08)]">
                ...
                <div className="space-y-2.5 sm:space-y-4">
                  ...
                  <h1 className="text-xl font-black tracking-tight text-[#222831] sm:text-3xl md:text-4xl">
                    {product.name}
                  </h1>
                  <div className="flex flex-wrap items-center gap-2.5 text-[#222831] sm:gap-4">
                    <span className="inline-flex items-center gap-1.5 text-xs sm:gap-2 sm:text-sm">
                      <Star size={14} className="text-[#222831] sm:h-4 sm:w-4" />
                  ...
                  <p className="max-w-2xl whitespace-pre-wrap break-words text-sm leading-6 text-[#222831] sm:text-base sm:leading-8">
                    {product.description}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4 rounded-2xl border border-[#EEEEEE] bg-white p-3 shadow-[0_10px_40px_rgba(0,0,0,0.06)] sm:space-y-6 sm:rounded-[32px] sm:p-6 sm:shadow-[0_20px_70px_rgba(0,0,0,0.08)]">
                <div className="space-y-1 sm:space-y-2">
                  ...
                  <div className="flex items-end gap-2 sm:gap-3">
                    <p className="text-3xl font-black text-[#222831] sm:text-5xl">
                      Rs {product.price.toLocaleString()}
                    </p>
                    <p className="pb-1 text-sm font-medium text-[#222831] line-through sm:pb-2 sm:text-xl">
                      Rs {Math.round(product.price * 1.4).toLocaleString()}
                    </p>
                  </div>
                </div>

                <div className="rounded-2xl bg-[#FFFFFF] p-3.5 text-xs text-[#222831] sm:rounded-3xl sm:p-5 sm:text-sm">
                  Premium quality product, ready to ship with fast delivery. Choose available color and size options at checkout.
                </div>
                ...
                <div className="grid gap-2.5 sm:gap-4">
                  ...
                  <button
                    ...
                    className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-2xl border border-[#EEEEEE] bg-white px-4 text-sm font-semibold text-[#222831] transition hover:bg-[#FFFFFF] sm:min-h-[56px] sm:gap-3 sm:rounded-3xl sm:px-5 sm:text-base"
                  >
                    Buy Now
                  </button>
                </div>
                ...
                <div className="rounded-2xl bg-[#FFFFFF] p-3.5 text-xs text-[#222831] sm:rounded-3xl sm:p-5 sm:text-sm">
                  <p className="font-semibold text-[#222831]">Product Details</p>
            ...
            <div className="mt-6 rounded-2xl border border-[#EEEEEE] bg-white p-4 shadow-[0_10px_40px_rgba(0,0,0,0.06)] sm:mt-12 sm:rounded-[32px] sm:p-8 sm:shadow-[0_20px_70px_rgba(0,0,0,0.08)]">
              <div className="mb-5 flex items-center gap-2 sm:mb-8 sm:gap-3">
                <MessageSquare size={18} className="text-[#222831] sm:h-6 sm:w-6" />
                <h2 className="text-lg font-black text-[#222831] sm:text-2xl">
                  Reviews &amp; Ratings
                </h2>
                <span className="ml-auto text-xs text-[#222831] sm:text-sm">
              ...
              {/* Rating Form */}
              {auth.username ? (
                <div className="mb-6 rounded-2xl border border-[#EEEEEE] bg-[#FFFFFF] p-4 sm:mb-10 sm:rounded-3xl sm:p-6">
                  <p className="mb-2.5 text-xs font-bold text-[#222831] sm:mb-3 sm:text-sm">
                    Write a Review
                  </p>
                  <div className="mb-2.5 flex gap-1 sm:mb-3">
                    ...
                    {star > 0 && (
                      <span className="ml-2 self-center text-xs text-[#222831] sm:text-sm">
                        {star} star{star > 1 ? "s" : ""}
                      </span>
                    )}
                  </div>
                ...
                <div className="mb-6 rounded-2xl border border-dashed border-[#EEEEEE] bg-[#FFFFFF] p-4 text-center sm:mb-10 sm:rounded-3xl sm:p-6">
                  <p className="text-xs text-[#222831] sm:text-sm">
                    Please{" "}
                    <button
                      onClick={() => router.push("/")}
                      className="font-semibold text-[#222831] hover:underline"
                    >
                      login
                    </button>{" "}
                    to write a review.
              ...
              {reviews.length > 0 ? (
                    ...
                    >
                      <div className="mb-1.5 flex items-center gap-2.5 sm:mb-2 sm:gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="truncate text-xs font-bold capitalize text-[#222831] sm:text-sm">
                            {review.user?.username || "User"}
                          </p>
                          <p className="text-[10px] text-[#222831] sm:text-xs">
                      ...
                      {review.comment && (
                        <p className="mt-1.5 text-xs leading-relaxed text-[#222831] sm:mt-2 sm:text-sm">
                          {review.comment}
                        </p>
                      )}
                    </div>
                ...
                <div className="py-8 text-center sm:py-10">
                  <Star size={32} className="mx-auto mb-2.5 text-[#222831] sm:h-10 sm:w-10 sm:mb-3" />
                  <p className="text-xs text-[#222831] sm:text-sm">
